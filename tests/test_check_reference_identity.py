...
def test_file_correct(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_env_var_wins_over_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    marker = tmp_path / "ci_reference.txt"
...
def test_env_var_invalid_exits_2(monkeypatch: pytest.MonkeyPatch, bad: str) -> None:
...
def test_file_wrong_value_exits_2(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    marker = tmp_path / "ci_reference.txt"
    marker.write_text("hg38_decoy\n", encoding="utf-8")
    monkeypatch.delenv("BSMN_REFERENCE", raising=False)
    monkeypatch.setenv("BSMN_REFERENCE_FILE", str(marker))
...
def test_file_missing_exits_2(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("BSMN_REFERENCE", raising=False)
    monkeypatch.setenv("BSMN_REFERENCE_FILE", str(tmp_path / "does_not_exist.txt"))
    with pytest.raises(SystemExit) as exc:
        r.check()
    assert exc.value.code == 2
