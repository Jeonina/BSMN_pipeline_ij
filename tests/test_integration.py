...
class TestPileupIntegration:
    ...
    def _run(self, pos: str, **kwargs):
        ...
        return pileup_base_counts(
            CRAM, REF, "chr22", pos, SAMTOOLS_SIF, **kwargs
        )
    ...
    def test_low_alt_position_returns_four_reads(self):
        ...
        assert depth == 4, (
            f"Expected depth=4 at chr22:{POS_LOW_ALT}, got {depth}"
        )
...
class TestFilterVariantsIntegration:
    ...
    def _filter(self, lines: str, **kwargs) -> str:
        ...
        out = io.StringIO()
        filter_variants(io.StringIO(lines), CRAM, REF, SAMTOOLS_SIF,
                        outfile=out, **kwargs)
        return out.getvalue()
    ...
    def test_germline_hom_alt_filtered_by_binomial(self):
        ...
        assert out == "", (
            f"Germline hom-alt variant (VAF=1.0) should be filtered; got: {out!r}"
        )
    ...
    def test_relaxed_threshold_with_low_count_still_filtered(self):
        ...
        out = self._filter(inp, min_alt=5, p_threshold=1.0)
        assert out == "", (
            f"4-read variant should be filtered by min_alt=5 regardless of p_threshold; got: {out!r}"
        )
...
class TestMappingOutputs:
    ...
    def test_cram_has_mapped_reads(self):
        ...
        result = subprocess.run(
            ["apptainer", "exec", SAMTOOLS_SIF,
             "samtools", "view", "-c", "-F", "4",
             "--input-fmt-option", f"reference={REF}",
             CRAM],
    ...
    def test_markdup_metrics_has_duplicate_rate(self):
        ...
        header_idx = next(
            i for i, l in enumerate(lines) if l.startswith("LIBRARY")
        )
